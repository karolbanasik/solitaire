...
class TransferRule:
    ...
    def king_on_empty(self):
        self.validness_trap_switch(self.bottom_transferred_card.rank == 'K')

    def transfer_to_face_up(self):
        self.validness_trap_switch(self.top_row_to_card.face_up)
...
if __name__ == '__main__':
    ...
    card_b = Card(suit= 'spades', rank='K')
    from row import Row
    row_a = Row()
    row_b = Row()
    #row_a.cards.append(card_a)
    row_b.cards.append(card_b)
    rule = TransferRule(row_b, row_a, 1)
    rule.check()
    pass